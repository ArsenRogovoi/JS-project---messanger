...
export const startUserPage = () => {
  ...
  if (typeof usersJSON === "string") {
    ...
    if (currentUser) {
      ...
      USER_PAGE_SEARCH_INPUT.addEventListener("input", () =>
        searchChat(currentUser?.chats?.length)
      );
    }
  }
};

export const showChat = (user: User, badgeCounter = 0) => {
...
export const showExistingChats = () => {
  ...
  // const existingChatsIds = data.CURRENT_USER.user.chats;
  ...
  if (typeof usersJSON === "string") {
    ...
    const specialChatArr: [Chat, Date][] = existingChatsIds?.map((chatId) => {
      let chats: Chat[];
    ...
    while (specialChatArr.length) {
      ...
      const user = chatToShow.participants.filter(
        (el) => el !== data?.CURRENT_USER?.user?.id
      )[0];
      let unreadMsgCounter = 0;
      chatToShow.messages.forEach((msg) => {
        if (!msg.hasRead) {
          if (msg.from !== data.CURRENT_USER?.user?.id) unreadMsgCounter++;
        }
      });
...
export const checkIfHasChangeInChats = () => {
  // DESCRIPTION:
  // This function runs when event 'storage' occurs
  // on element USER_PAGE_CHAT_AREA. This function
  // will check if there are changes in local storage and if are
  // will check if there is a need to render chat links.
  let chats: Chat[];
  let users: User[];
  const chatsJSON = localStorage.getItem("chats");
  const usersJSON = localStorage.getItem("users");
    ...
    chats.forEach((ch) => {
      if (currentUser?.chats.includes(ch.id)) {
        ...
        data.CURRENT_USER.datesOfLastMessages.forEach((el) => {
          if (
            Date.parse(el.toString()) > Date.parse(lastMsgOfCh.date.toString())
          )
            ifRender = false;
        });
